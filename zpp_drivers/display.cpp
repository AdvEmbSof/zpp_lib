...
ZephyrResult Display::init() {
    ...
    LOG_INF("Display capabilities: x_res %d y_res %d", _displayCapabilities.x_resolution, _displayCapabilities.y_resolution);
...
void Display::setBackColor(uint32_t color) {
  _backColor = color;
}

void Display::setFont(const Font* pFont) {
  _pFont = pFont;
}

const Display::Font* Display::getFont() const {
  return _pFont;
}
...
void Display::drawStringAt(uint32_t xPos, uint32_t yPos, const char* text, AlignMode mode) {
    // Get the text size
    ...
    switch (mode) {
        case AlignMode::CENTER_MODE: {
            refcolumn = xPos + (((int32_t) nbrOfCharPerLine - (int32_t) nbrOfChars) * _pFont->width) / 2;
            break;
        }
